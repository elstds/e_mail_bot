...
async def get_added_user_type(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите ID нового пользователя:", reply_markup=keyboards.default.admin_kb.admin_kb_menu)
...
async def get_added_user_id(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"{id_} не является иденификатором пользователя")
    finally:
